      ...
      ->Option.flatMap(pipeline =>
          {
            ...
            pipeline,
            builds:
              Js.Array.sliceFrom(2, lines)->Array.keepMap(buildFromMessage),
...
module Zuul = {
  ...
  let fromMessage =
    fromMessage(content =>
      headerRe->firstMatch(content)->Option.flatMap(v => v->Some->Some)
    );
};
...
module Results = {
  ...
  let addResult = (history: list(t), current: result): list(t) => {
    let rec go = (xs, replaced: bool, acc: list(t)) =>
      switch (xs) {
      | [] =>
        // if result was not replaced, add it to the accumulator
        replaced ? acc : acc->List.add({count: 1, latest: current})
      | [x, ...xs] =>
        // if previous result exists, replace and increase the recheck count
        x.latest.name == current.name && x.latest.pipeline == current.pipeline
          ? xs->go(
              true,
              acc->List.add({count: x.count + 1, latest: current}),
            )
